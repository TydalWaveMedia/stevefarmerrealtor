...
*Requirement: To activate the FlexMLS&reg; IDX plugin, you must already be approved by your local MLS and FBS for FlexMLS&reg; IDX. Currently, FBS only provides IDX services to agents and brokers who are members of an MLS that uses FlexMLS&reg; Web.  Accordingly, if you are not a member of an MLS organization that uses FlexMLS&reg; Web, this plugin won't be useful to you.*

The FlexMLS&reg; IDX plugin includes six widgets you can add to your side bar, pages and posts.  The widgets are:
...
*Live data from the MLS:* The IDX and market statistics widgets are all updated with IDX listing and statistics information live from the FlexMLS&reg; Web system used by your local MLS.
...
The easiest way to install the plugin is to use the Add New Plugins function within WordPress and search for FlexMLS&reg; IDX.

Otherwise:

1. Upload the unzipped `flexmls-idx` folder to the `/wp-content/plugins/` directory
1. Activate the plugin through the 'Plugins' menu in WordPress
1. Go to **FlexMLS&reg; IDX > Settings** in WordPress and enter your API key information
...
= Do I need to be approved for FlexMLS&reg; IDX by my local MLS and FBS in order to use this plugin? =
Yes, this plugin requires an API key that is only available to approved members.

= How do I obtain API credentials? =
Please call FBS Broker Agent Services at 866-320-9977 or email <a href='mailto:idxwordpress@flexmls.com'>idxwordpress@flexmls.com</a> to purchase a key to activate your plugin.
