...
if ($query->found_posts > 0) {
	...
	?><div<?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> class="sc_properties sc_properties_<?php 
		...
		if ($args['slider']) {
			...
			?><div class="sc_properties_columns 1 sc_item_columns sc_item_columns_<?php
							echo esc_attr($args['columns']);
							echo ' '.esc_attr(trx_addons_get_columns_wrap_class()) . ' columns_padding_bottom';
                            echo((($args['bg_color_column'])==1) ? ' bg_color_white' : '' );
		...
		while ( $query->have_posts() ) { $query->the_post();
			trx_addons_get_template_part(array(
											'cpt/properties/tpl.'.trx_addons_esc($args['type']).'-item.php',
											'cpt/properties/tpl.properties.default-item.php'
											),
