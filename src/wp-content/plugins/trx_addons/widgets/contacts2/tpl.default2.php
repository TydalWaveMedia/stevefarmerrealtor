...
$args = get_query_var('trx_addons_args_widget_contacts2');
extract($args);
...
if ($show_info || !empty($googlemap)) {
	...
	if (!empty($address) || !empty($phone) || !empty($email)) {
        ...
        if (!empty($phone) || !empty($email)) {
            ...
            if (!empty($phone)) {
                ?><span class="contacts_phone"></span>
				<span class="phone"><?php echo str_replace('|', "<br><span>", $phone); ?></span></span><?php
            }
            if (!empty($email)) {
		...
		if (!empty($address)) {
			...
			?><span class="contacts_address"></span>
			<span class="address"><?php echo str_replace('|', "<br><span>", $address); ?></span></span><?php
