...
// Load widget
if (!function_exists('trx_addons_widget_contacts2_load')) {
	add_action( 'widgets_init', 'trx_addons_widget_contacts2_load' );
	function trx_addons_widget_contacts2_load() {
		register_widget('trx_addons_widget_contacts2');
	}
}

// Widget Class
class trx_addons_widget_contacts2 extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_contacts2', 'description' => esc_html__('Contacts - logo and short description, address, phone and email', 'trx_addons'));
		parent::__construct( 'trx_addons_widget_contacts2', esc_html__('ThemeREX Addons - Contacts 2', 'trx_addons'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		trx_addons_get_template_part('widgets/contacts2/tpl.default2.php',
									'trx_addons_args_widget_contacts2',
...
if ( !function_exists( 'trx_addons_widget_contacts2_load_scripts_front' ) ) {
	add_action("wp_enqueue_scripts", 'trx_addons_widget_contacts2_load_scripts_front');
	function trx_addons_widget_contacts2_load_scripts_front() {
		if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			wp_enqueue_style( 'trx_addons-widget_contacts2', trx_addons_get_file_url('widgets/contacts2/contacts2.css'), array(), null );
		}
	}
}
...
if ( !function_exists( 'trx_addons_widget_contacts2_merge_styles' ) ) {
	add_action("trx_addons_filter_merge_styles", 'trx_addons_widget_contacts2_merge_styles');
	function trx_addons_widget_contacts2_merge_styles($list) {
		$list[] = 'widgets/contacts2/contacts2.css';
		return $list;
	}
}
...
*/
if ( !function_exists( 'trx_addons_sc_widget_contacts2' ) ) {
	function trx_addons_sc_widget_contacts2($atts, $content=null){
		$atts = trx_addons_sc_prepare_atts('trx_widget_contacts2', $atts, array(
			// Individual params
			"title" => "",
			"logo" => "",
			"logo_retina" => "",
		...
		$type = 'trx_addons_widget_contacts2';
		$output = '';
		global $wp_widget_factory, $TRX_ADDONS_STORAGE;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_contacts2'
								. (trx_addons_exists_visual_composer() ? ' vc_widget_contacts2 wpb_content_element' : '')
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
								. '"'
							. ($css ? ' style="'.esc_attr($css).'"' : '')
						. '>';
			ob_start();
			the_widget( $type, $atts, trx_addons_prepare_widgets_args($TRX_ADDONS_STORAGE['widgets_args'], $id ? $id.'_widget' : 'widget_contacts2', 'widget_contacts2') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('trx_addons_sc_output', $output, 'trx_widget_contacts2', $atts, $content);
	}
}


// Add [trx_widget_contacts] in the VC shortcodes list
if (!function_exists('trx_addons_sc_widget_contacts2_add_in_vc')) {
	function trx_addons_sc_widget_contacts2_add_in_vc() {
		
		if (!trx_addons_exists_visual_composer()) return;
		
		add_shortcode("trx_widget_contacts2", "trx_addons_sc_widget_contacts2");
		
		vc_lean_map("trx_widget_contacts2", 'trx_addons_sc_widget_contacts2_add_in_vc_params');
		class WPBakeryShortCode_Trx_Widget_Contacts2 extends WPBakeryShortCodesContainer {}
	}
	add_action('init', 'trx_addons_sc_widget_contacts2_add_in_vc', 20);
}

// Return params
if (!function_exists('trx_addons_sc_widget_contacts2_add_in_vc_params')) {
	function trx_addons_sc_widget_contacts2_add_in_vc_params() {
		return apply_filters('trx_addons_sc_map', array(
				"base" => "trx_widget_contacts2",
				"name" => esc_html__("Contacts2", 'trx_addons'),
				...
				"class" => "trx_widget_contacts2",
				"content_element" => true,
				'is_container' => true,
				'as_child' => array('except' => 'trx_widget_contacts2'),
