...
if ( !function_exists('good_homes_need_page_title') ) {
	function good_homes_need_page_title() {
		return !is_front_page() && apply_filters('good_homes_filter_need_page_title', true);
	}
}
...
if ( !function_exists('good_homes_show_layout') ) {
	function good_homes_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('good_homes_get_logo_image') ) {
	function good_homes_get_logo_image($type='') {
		$logo_image = '';
		if (good_homes_get_retina_multiplier(2) > 1)
			$logo_image = good_homes_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = good_homes_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('good_homes_get_header_video') ) {
	function good_homes_get_header_video() {
		$video = '';
		if (apply_filters('good_homes_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (good_homes_check_theme_option('header_video')) {
				$video = good_homes_get_theme_option('header_video');
...
if ( !function_exists('good_homes_show_post_meta') ) {
	function good_homes_show_post_meta($args=array()) {
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				// Post categories
				if ($comp == 'categories') {
					$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('good_homes_filter_get_post_categories', '');
					if (!empty($cats)) {
						?>
						<span class="post_meta_item post_categories"><?php good_homes_show_layout($cats); ?></span>
						<?php
					}

				// Post date
				} else if ($comp == 'date') {
					$dt = apply_filters('good_homes_filter_get_post_date', good_homes_get_date());
					if (!empty($dt)) {
						?>
				...
				// Post counters
					...
					good_homes_show_layout(good_homes_get_post_counters($args['counters']));
	
				// Socials share
				} else if ($comp == 'share') {
					good_homes_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'good-homes'),
				...
				// Edit page link
					...
					edit_post_link( esc_html__( 'Edit', 'good-homes' ), '<span class="post_meta_item post_edit">', '</span>' );
				}
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('good_homes_show_post_featured') ) {
	function good_homes_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => good_homes_get_theme_option('image_hover'),	// Hover effect
		...
		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : good_homes_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? good_homes_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = good_homes_get_post_image();
				if (!empty($image)) 
					$image = good_homes_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . good_homes_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="http://schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$good_homes_attr = good_homes_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						good_homes_hovers_add_icons($args['hover']);
					}
					...
					if (($output=good_homes_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						good_homes_show_layout($output);
					...
					if (!empty($image)) {
						...
						good_homes_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if ($post_format == 'video' && !$args['thumb_only']) {
					$video = good_homes_get_post_video('', false);
					if (empty($video))
						$video = good_homes_get_post_iframe('', false);
					if (!empty($video)) {
						if ( $has_thumb ) {
							$video = good_homes_make_video_autoplay($video);
							?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
						}
						?><div class="post_video video_frame"><?php 
							if ( !$has_thumb ) {
								good_homes_show_layout($video);
							}
						?></div><?php
					}
				...
				if ($post_format == 'audio' && !$args['thumb_only']) {
					$audio = good_homes_get_post_audio('', false);
					if (empty($audio))
						$audio = good_homes_get_post_iframe('', false);
					if (!empty($audio)) {
						//Show metadata (for the future version)
						if (false && function_exists('wp_read_audio_metadata')) {
							$src = good_homes_get_post_audio($audio);
						...
						?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
							...
							$media_author = good_homes_get_theme_option('media_author', '', false, get_the_ID());
							$media_title = good_homes_get_theme_option('media_title', '', false, get_the_ID());
							if ( !empty($media_author) && !good_homes_is_inherit($media_author) ) {
								?><div class="post_audio_author"><?php good_homes_show_layout($media_author); ?></div><?php
							}
							if ( !empty($media_title) && !good_homes_is_inherit($media_title) ) {
								?><h5 class="post_audio_title"><?php good_homes_show_layout($media_title); ?></h5><?php
							}
							// Display audio
							good_homes_show_layout($audio); 
						?></div><?php
					}
				}
				
				// Put optional info block over the thumb
				good_homes_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				good_homes_show_layout($post_info);
...
if ( !function_exists('good_homes_get_no_image') ) {
	function good_homes_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = good_homes_get_theme_option( 'no_image' );
			if (empty($img)) $img = good_homes_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('good_homes_add_bg_in_post_nav') ) {
	function good_homes_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = good_homes_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), good_homes_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), good_homes_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'good_homes-main', $css );
	}
}

// Show related posts
if ( !function_exists('good_homes_show_related_posts') ) {
	function good_homes_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'good-homes');
				?></h3>
...
if ( !function_exists('good_homes_show_portfolio_posts') ) {
	function good_homes_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? good_homes_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = good_homes_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = good_homes_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the good_homes_ajax_get_posts action
if ( !function_exists( 'good_homes_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_good_homes_ajax_get_posts',			'good_homes_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_good_homes_ajax_get_posts',	'good_homes_ajax_get_posts_callback');
	function good_homes_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			good_homes_storage_set('blog_mode', 'blog');
			good_homes_storage_set('options_meta', get_post_meta($id, 'good_homes_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => good_homes_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'good-homes');
		}
...
// Show pagination
if ( !function_exists('good_homes_show_pagination') ) {
	function good_homes_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = good_homes_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'good-homes' ),
				'next_text' => esc_html__( '>', 'good-homes' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'good-homes' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'good-homes'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'good-homes') : esc_html__('Newest posts', 'good-homes') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'good-homes') : esc_html__('Older posts', 'good-homes'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
