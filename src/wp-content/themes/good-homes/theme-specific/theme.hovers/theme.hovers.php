...
if (!function_exists('good_homes_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'good_homes_hovers_theme_setup3', 3 );
	function good_homes_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		good_homes_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'good-homes'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'good-homes') ),
				"std" => 'fade',
				"options" => array(
					'default'		=> esc_html__('Fade',				'good-homes'),
//					'arrow'			=> esc_html__('Arrow',				'good-homes'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'good-homes'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'good-homes') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'good-homes'),
					'icon'	=> esc_html__('Icon',	'good-homes'),
					'icons'	=> esc_html__('Icons',	'good-homes'),
					'zoom'	=> esc_html__('Zoom',	'good-homes'),
					'fade'	=> esc_html__('Fade',	'good-homes'),
					'slide'	=> esc_html__('Slide',	'good-homes'),
					'pull'	=> esc_html__('Pull',	'good-homes'),
					'border'=> esc_html__('Border',	'good-homes')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('good_homes_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'good_homes_hovers_theme_setup9', 9 );
	function good_homes_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'good_homes_hovers_frontend_scripts', 1010 );
		add_filter( 'good_homes_filter_localize_script','good_homes_hovers_localize_script' );
		add_filter( 'good_homes_filter_merge_scripts',	'good_homes_hovers_merge_scripts' );
		add_filter( 'good_homes_filter_merge_styles',	'good_homes_hovers_merge_styles' );
		add_filter( 'good_homes_filter_get_css', 		'good_homes_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'good_homes_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'good_homes_hovers_frontend_scripts', 1010 );
	function good_homes_hovers_frontend_scripts() {
		if ( good_homes_is_on(good_homes_get_theme_option('debug_mode')) && good_homes_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'good_homes-hovers', good_homes_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( good_homes_is_on(good_homes_get_theme_option('debug_mode')) && good_homes_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'good_homes-hovers',  good_homes_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('good_homes_hovers_merge_scripts')) {
	//Handler of the add_filter( 'good_homes_filter_merge_scripts', 'good_homes_hovers_merge_scripts' );
	function good_homes_hovers_merge_scripts($list) {
...
if (!function_exists('good_homes_hovers_merge_styles')) {
	//Handler of the add_filter( 'good_homes_filter_merge_styles', 'good_homes_hovers_merge_styles' );
	function good_homes_hovers_merge_styles($list) {
...
if (!function_exists('good_homes_hovers_localize_script')) {
	//Handler of the add_filter( 'good_homes_filter_localize_script','good_homes_hovers_localize_script' );
	function good_homes_hovers_localize_script($arr) {
		$arr['button_hover'] = good_homes_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('good_homes_hovers_add_icons') ) {
	function good_homes_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'good-homes') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'good-homes');
						else
							esc_html_e('Details', 'good-homes');
					}
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'good_homes_hovers_get_css' ) ) {
	//Handler of the add_filter( 'good_homes_filter_get_css', 'good_homes_hovers_get_css', 10, 4 );
	function good_homes_hovers_get_css($css, $colors, $fonts, $scheme='') {
