...
------------------------------------------------------------------------------- */
if (!function_exists('good_homes_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'good_homes_trx_addons_theme_specific_components');
	function good_homes_trx_addons_theme_specific_components($enable=false) {
		return false;
	}
}

if (!function_exists('good_homes_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'good_homes_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'good_homes_trx_addons_theme_specific_setup1', 8 );
	function good_homes_trx_addons_theme_specific_setup1() {
		if (good_homes_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',				'good_homes_trx_addons_cv_enable');
			add_filter( 'trx_addons_cpt_list',				'good_homes_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',				'good_homes_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',			'good_homes_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'good_homes_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'good_homes_trx_addons_cv_enable');
	function good_homes_trx_addons_cv_enable($enable=false) {
...
// CPT
if ( !function_exists( 'good_homes_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'good_homes_trx_addons_cpt_list');
	function good_homes_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'good_homes_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'good_homes_trx_addons_sc_list');
	function good_homes_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'good_homes_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'good_homes_trx_addons_widgets_list');
	function good_homes_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'good_homes_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'good_homes_trx_addons_menu_cache');
	function good_homes_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('good_homes_trx_addons_localize_script')) {
	...
	function good_homes_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = good_homes_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'good_homes_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'good_homes_trx_addons_sc_type', 10, 2);
	function good_homes_trx_addons_sc_type($list, $sc) {
...
if ( !function_exists( 'good_homes_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'good_homes_trx_addons_sc_atts', 10, 2);
	function good_homes_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'good_homes_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'good_homes_trx_addons_sc_map', 10, 2);
	function good_homes_trx_addons_sc_map($params, $sc) {
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'good-homes'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'good-homes') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array(
				esc_html__('Default', 'good-homes') => 'default',
				esc_html__('Link 2', 'good-homes') => 'link2',
				esc_html__('Link 3', 'good-homes') => 'link3',
				esc_html__('Dark', 'good-homes') => 'dark'
			),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'good_homes_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'good_homes_trx_addons_sc_output', 10, 4);
	function good_homes_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !good_homes_is_inherit($atts['color_style']))
...
if ( !function_exists( 'good_homes_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'good_homes_trx_addons_sc_item_title_tag');
	function good_homes_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'good_homes_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'good_homes_trx_addons_sc_item_button_args', 10, 3);
	function good_homes_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'good_homes_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'good_homes_trx_addons_slider_title', 10, 2 );
	function good_homes_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'good_homes_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'good_homes_trx_addons_sc_googlemap_styles');
	function good_homes_trx_addons_sc_googlemap_styles($list) {
		$list[esc_html__('Dark', 'good-homes')] = 'dark';
		return $list;
	}
}
...
if ( !function_exists( 'good_homes_trx_addons_editor_init' ) ) {
	if (is_admin()) add_filter( 'tiny_mce_before_init', 'good_homes_trx_addons_editor_init', 11);
	function good_homes_trx_addons_editor_init($opt) {
		if (good_homes_exists_trx_addons()) {
			...
			if (false && !empty($opt['style_formats'])) {
				...
				if (is_array($style_formats) && count($style_formats)>0 ) {
					foreach ($style_formats as $k=>$v) {
						if ( $v['title'] == esc_html__('List styles', 'good-homes') ) {
							$style_formats[$k]['items'][] = array(
										'title' => esc_html__('Arrow', 'good-homes'),
										'selector' => 'ul',
...
if ( !function_exists( 'good_homes_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'good_homes_filter_allow_override_header_image', 'good_homes_trx_addons_allow_override_header_image' );
	function good_homes_trx_addons_allow_override_header_image($allow) {
		return good_homes_is_team_page() || good_homes_is_portfolio_page() ? false : $allow;
	}
}

// Hide sidebar on the team and portfolio pages
if ( !function_exists( 'good_homes_trx_addons_sidebar_present' ) ) {
	add_filter( 'good_homes_filter_sidebar_present', 'good_homes_trx_addons_sidebar_present' );
	function good_homes_trx_addons_sidebar_present($present) {
		return !is_single() && (good_homes_is_team_page() || good_homes_is_portfolio_page()) ? false : $present;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'good_homes_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'good_homes_trx_addons_thumb_size', 10, 2);
	function good_homes_trx_addons_thumb_size($thumb_size='', $type='') {
		if ($type == 'team-default')
			$thumb_size = good_homes_get_thumb_size('avatar');
		return $thumb_size;
	}
}
...
if (!function_exists('good_homes_trx_addons_meta_box_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'good_homes_trx_addons_meta_box_fields', 10, 2);
	function good_homes_trx_addons_meta_box_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'good-homes'),
				"desc" => wp_kses_data( __("Team member's email", 'good-homes') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
            $mb['phone'] = array(
                "title" => esc_html__("Phone",  'good-homes'),
                "desc" => wp_kses_data( __("Team member's phone", 'good-homes') ),
                "std" => "",
                "details" => true,
                "type" => "text"
            );
