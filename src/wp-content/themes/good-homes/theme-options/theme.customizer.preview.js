...
( function( $ ) {
	...
	var $style = $('#good_homes-color-scheme-css'),
		api = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		$style = $('head').append( '<'+'style type="text/css" id="good_homes-color-scheme-css" />' ).find('#good_homes-color-scheme-css');
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-other-controls', function( obj ) {
			...
			if (id == 'body_style') {
				...
				$('.sc_layouts_logo .logo_text').html( good_homes_prepare_macros(val) );

			} else if (id == 'blogdescription') {
				$( '.sc_layouts_logo .logo_slogan' ).html( good_homes_prepare_macros(val) );

			} else if (id == 'copyright') {
				$( '.copyright_text' ).html( good_homes_prepare_macros(val) );
