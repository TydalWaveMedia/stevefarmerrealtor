/* global good_homes_color_schemes, good_homes_dependencies, Color */
...
( function( api ) {
	...
	for (var i in good_homes_color_schemes) {
		cssTemplate[i] = wp.template( 'good_homes-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'good_homes-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {

		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+good_homes_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(good_homes_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', good_homes_customizer_update_load_fonts);		

		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', good_homes_customizer_actions);

		// Check dependencies in the each section
		jQuery('#customize-theme-controls .control-section').each(function () {
			good_homes_customizer_check_dependencies(jQuery(this));
		});

		// Init color controls
		jQuery('#customize-theme-controls .good_homes_scheme_editor_selector').on('change', function(e) {
			good_homes_customizer_change_color_scheme(jQuery(this).val());
		});
		jQuery('#customize-theme-controls .good_homes_scheme_editor_colors input').on('change', function(e) {
			good_homes_customizer_change_field_colors(jQuery(this), jQuery(this).val());
			good_homes_customizer_update_color_scheme(jQuery(this).attr('name'), jQuery(this).val());
		});
		// Color picker
		good_homes_color_picker();
		jQuery('#customize-theme-controls .iColorPicker').each(function() {
			var clr = jQuery(this).val();
			good_homes_customizer_change_field_colors(jQuery(this), clr);
		});
		jQuery('#customize-theme-controls').on('focus', '.iColorPicker', function (e) {
			good_homes_color_picker_show(null, jQuery(this), function(fld, clr) {
				good_homes_customizer_change_field_colors(fld, clr);
	...
	api.bind('change', function(obj) {
		...
		good_homes_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		good_homes_customizer_refresh_preview(obj);
	});

	// Check for dependencies
	function good_homes_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			var id = jQuery(this).attr('id');
			if (id == undefined) return;
			id = id.replace('customize-control-', '');
			var depend = false;
			for (var fld in good_homes_dependencies) {
				if (fld == id) {
					depend = good_homes_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('good_homes_options_no_use');
				} else {
					jQuery(this).hide().addClass('good_homes_options_no_use');
				}
			}
		});
	}
	...
	function good_homes_customizer_refresh_preview(obj) {
        console.log('good_homes_customizer_refresh_preview', obj.id);
		...
		if (updateCSS) {
			...
			// Any theme fonts parameter is changed
				...
				for (opt in good_homes_theme_fonts) {
					for (rule in good_homes_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							good_homes_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) good_homes_customizer_update_css();
		}
	...
	// Actions buttons
	function good_homes_customizer_actions(e) {
	...
	function good_homes_customizer_change_color_scheme(value) {
		updateCSS = false;
		for (var opt in good_homes_color_schemes[value].colors) {
			var fld = jQuery('#customize-theme-controls .good_homes_scheme_editor_colors input[name="'+opt+'"]');
			if (fld.length == 0) continue;
			fld.val( good_homes_color_schemes[value].colors[opt] );
			good_homes_customizer_change_field_colors(fld, good_homes_color_schemes[value].colors[opt]);
		}
		updateCSS = true;
		good_homes_customizer_update_css();
	}

	// Store new value in the color table
	function good_homes_customizer_update_color_scheme(opt, value) {
		good_homes_color_schemes[jQuery('.good_homes_scheme_editor_selector').val()].colors[opt] = value;
		api('scheme_storage').set(good_homes_serialize(good_homes_color_schemes))
	}
	
	// Change color in the field
	function good_homes_customizer_change_field_colors(fld, clr) {
		var hsb = good_homes_hex2hsb(clr);
		fld.css({
			'backgroundColor': clr,
	...
	function good_homes_customizer_update_theme_fonts(opt, rule, value) {
		good_homes_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function good_homes_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i<=good_homes_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in good_homes_theme_fonts) {
	...
	function good_homes_customizer_update_css() {
		...
		var fonts = good_homes_customizer_add_theme_fonts(good_homes_theme_fonts);

		// Make styles and add into css
		css += good_homes_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));
		// Add colors
		for (var scheme in good_homes_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in good_homes_color_schemes[scheme].colors) {
				colors[i] = good_homes_color_schemes[scheme].colors[i];
			}
            var tmpl = jQuery( '#tmpl-good_homes-color-scheme-' + scheme ).html().trim();
            colors = good_homes_customizer_add_theme_colors(colors);
            for (var i in colors) {
	...
	function good_homes_customizer_add_theme_fonts(fonts) {
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' && fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + good_homes_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' && fonts[tag]['margin-top'] != '' && fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + good_homes_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' && fonts[tag]['margin-bottom'] != '' && fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + good_homes_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function good_homes_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function good_homes_customizer_prepare_html_value(val) {
