...
jQuery(document).ready(function() {
	...
	function good_homes_ready_actions() {
		...
		if (document.documentElement.className.indexOf(GOOD_HOMES_STORAGE['site_scheme'])==-1)
			document.documentElement.className += ' ' + GOOD_HOMES_STORAGE['site_scheme'];
		...
		if (GOOD_HOMES_STORAGE['background_video'] && jQuery('.top_panel.with_bg_video').length > 0 && window.Bideo) {
			...
			setTimeout(function() {
				...
				bv.init({
					...
					src: [
						{
							src: GOOD_HOMES_STORAGE['background_video'],
							type: 'video/'+good_homes_get_file_ext(GOOD_HOMES_STORAGE['background_video'])
						}
					],
		...
		if (rows.length > 0) {
			...
			rows.each(function() {
				if (!jQuery(this).next().hasClass('good_homes_layouts_row_fixed_placeholder'))
					jQuery(this).after('<div class="good_homes_layouts_row_fixed_placeholder"></div>');
			});
		...
		jQuery('.menu_mobile').on('click', 'li a, li a .open_child_menu', function(e) {
			...
			if (!jQuery(this).hasClass('open_child_menu') && good_homes_is_local_link($a.attr('href')))
		...
		// Change display mode
		jQuery('.woocommerce,.woocommerce-page').on('click', '.good_homes_shop_mode_buttons a', function(e) {
			var mode = jQuery(this).hasClass('woocommerce_thumbs') ? 'thumbs' : 'list';
			good_homes_set_cookie('good_homes_shop_mode', mode, 365);
		...
		if (GOOD_HOMES_STORAGE['stretch_tabs_area'] > 0)
			jQuery('.single-product .woocommerce-tabs').wrap('<div class="trx-stretch-width"></div>');
		// Wrap stretch blocks
		jQuery('.trx-stretch-width').wrap('<div class="trx-stretch-width-wrap"></div>');
		jQuery('.trx-stretch-width').after('<div class="trx-stretch-width-original"></div>');
		good_homes_stretch_width();
		...
		jQuery('.nav-links-more a').on('click', function(e) {
			...
			if (panel.length == 0) {															// Load simple page content
				jQuery.get(location.href, {
					...
					good_homes_loadmore_add_items(jQuery('.content .posts_container').eq(0),
				...
				jQuery.post(GOOD_HOMES_STORAGE['ajax_url'], {
					nonce: GOOD_HOMES_STORAGE['ajax_nonce'],
					action: 'good_homes_ajax_get_posts',
			...
			function good_homes_loadmore_add_items(container, items) {
				...
				if (page+1 >= max_page)
					...
					GOOD_HOMES_STORAGE['load_more_link_busy'] = false;
	...
	function good_homes_check_layout() {
		...
		if (GOOD_HOMES_STORAGE['mobile_device'] || good_homes_browser_is_mobile()) 
			...
			if (window.mejs) {
				...
				cont.find('audio:not(.inited),video:not(.inited)').each(function() {
					...
					if (jQuery(this).parents('.mejs-mediaelement').length == 0 
							&& (GOOD_HOMES_STORAGE['init_all_mediaelements'] 
				...
				setTimeout(function() { good_homes_init_media_elements(cont); }, 400);
		}
	}
	...
	function good_homes_tabs_ajax_content_loader(panel, page, oldPanel) {
		if (panel.html().replace(/\s/g, '')=='') {
			...
			panel.find('> *').addClass('good_homes_tab_content_remove');
	...
	function good_homes_comments_validate(form) {
		...
		var comments_args = {
			error_message_text: GOOD_HOMES_STORAGE['strings']['error_global'],	// Global error message text (if don't write in checked field)
			error_message_show: true,									// Display or not error message
			error_message_time: 4000,									// Error message display time
			error_message_class: 'good_homes_messagebox good_homes_messagebox_style_error',	// Class appended to error message block
			error_fields_class: 'error_field',							// Class appended to error fields
			exit_after_first_error: false,								// Cancel validation and exit after first error
			rules: [
				{
					field: 'comment',
					min_length: { value: 1, message: GOOD_HOMES_STORAGE['strings']['text_empty'] },
					max_length: { value: GOOD_HOMES_STORAGE['comment_maxlength'], message: GOOD_HOMES_STORAGE['strings']['text_long']}
				}
			]
		};
		if (form.find('.comments_author input[aria-required="true"]').length > 0) {
			comments_args.rules.push(
				{
					field: 'author',
					min_length: { value: 1, message: GOOD_HOMES_STORAGE['strings']['name_empty']},
					max_length: { value: 60, message: GOOD_HOMES_STORAGE['strings']['name_long']}
				}
			);
		}
		if (form.find('.comments_email input[aria-required="true"]').length > 0) {
			comments_args.rules.push(
				{
					field: 'email',
					min_length: { value: 1, message: GOOD_HOMES_STORAGE['strings']['email_empty']},
					max_length: { value: 60, message: GOOD_HOMES_STORAGE['strings']['email_long']},
					mask: { value: GOOD_HOMES_STORAGE['email_mask'], message: GOOD_HOMES_STORAGE['strings']['email_not_valid']}
				}
			);
		}
