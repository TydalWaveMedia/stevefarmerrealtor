...
(function(){
			...
			case "object":
				if (mixed_val == null)
					...
					for (var i=0; i<mixed_val.length; i++) {
						...
						if (ser)
							map.push(good_homes_serialize(idxobj.idx) + ser);
					}                                      
					...
					for (var prop in mixed_val) {
						...
						if (ser)
							props.push(good_homes_serialize(prop) + ser);
					}
					if (obj_to_array)
	...
	window.good_homes_get_date_difference = function(dt1) {
		...
		if (dt2 == '') {
			...
			var a2 = good_homes_parse_date(dt2);
	...
	/*
	// Usage example:
	var error = good_homes_form_validate(jQuery(form_selector), {		// -------- Options ---------
	...
	*/
		...
		if (obj!==null) {
			for (var prop in obj) {
				...
				if (level<recursive && (typeof(obj[prop])=='object' || typeof(obj[prop])=='array') && obj[prop]!=obj)
					dispStr += addStr + prop + '=' + good_homes_debug_object(obj[prop], recursive, showMethods, level+1);
				else {
					try {
						curStr = "" + obj[prop];
					} catch (e) {
	...
	window.dco===undefined && (window.dco = function(s,r) { console.log(good_homes_debug_object(s,r)); });
	window.dal===undefined && (window.dal = function(s) { if (GOOD_HOMES_STORAGE['user_logged_in']) alert(s); });
	window.dao===undefined && (window.dao = function(s,r) { if (GOOD_HOMES_STORAGE['user_logged_in']) alert(good_homes_debug_object(s,r)); });
	window.ddl===undefined && (window.ddl = function(s) { good_homes_debug_log(s); });
	window.ddo===undefined && (window.ddo = function(s,r) { good_homes_debug_log(good_homes_debug_object(s,r)); });
